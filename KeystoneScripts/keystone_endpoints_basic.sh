...
# Host address
HOST_IP=10.10.10.51
EXT_HOST_IP=172.16.9.151
CINDER_HOST_IP=10.10.10.54
SWIFT_HOST_IP=10.10.10.55
...
create_endpoint () {
  case $1 in
    ...
    ;;
    volume)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$CINDER_HOST_IP"':8776/v1/$(tenant_id)s' --adminurl 'http://'"$CINDER_HOST_IP"':8776/v1/$(tenant_id)s' --internalurl 'http://'"$CINDER_HOST_IP"':8776/v1/$(tenant_id)s'
    ;;
    image)
    ...
    ;;
    object-store)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl  'http://'"$SWIFT_HOST_IP"':8888/v1/AUTH_$(tenant_id)s' --adminurl 'http://'"$SWIFT_HOST_IP"':8888/v1' --internalurl 'http://'"$SWIFT_HOST_IP"':8888/v1/AUTH_$(tenant_id)s'
    ;;
  esac
}
